            ...
            if( $countFiles > 0 ) {
                for ($i = 0; $i < $countFiles; $i++ ) {
                    if( isset( $this->Files['name'][$i] ) && trim( $this->Files['name'][$i] ) != '' ) {
                        $extensaoAux = explode( '.', $this->Files['name'][$i] );
                        $extensaoAux = end($extensaoAux) ;
                        $extensao  =  strtolower($extensaoAux);
                        $length    =  $this->Files['size'][$i];
                        $name      =  $this->Files['name'][$i];
                        if( array_search($extensao, $this->Extensions ) === false ) {
                            $this->Response['status']   =   $this->Files['name'][$i] . ' - Arquivo não permitido';
                            $this->Response['code']     =   102;
                            break;
                        }else if( $length > $this->Length ) {
                            $this->Response['status']   =   $this->Files['name'][$i] . ' excede o limite de tamanho de: 10 MB';
                            $this->Response['code']     =   102;
                            break;
                        }
                    }
                        ...
                        if( isset( $this->Files['name'][$i] ) && trim( $this->Files['name'][$i] ) != '' ) {
                            if( !isset( $this->Response['code'] ) ) {
                                ...
                                if( move_uploaded_file( $this->Files['tmp_name'][$i], $file ) ) {
                                    ...
                                    $this->Response[$i]['nome']    =  $this->Files['name'][$i];
                                }else {
                                    $this->Response['status']   =   $this->Files['name'][$i] . ' - Ocorreu um erro ao tentar salvar o arquivo';
                                    $this->Response['code']     =   102;
